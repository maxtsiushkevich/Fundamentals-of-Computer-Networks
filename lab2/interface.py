...
def create_label(text, relx, rely, root):
    label = Label(root, text=text)
    label.place(relx=relx, rely=rely)
    return label


def create_baudrate_combobox(parent, values, default_value, relx, rely, is_input_combobox, change_baudrate):
...
def open_port_entry_window(root, set_port_names):
